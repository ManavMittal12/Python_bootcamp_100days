...
def find_cheap_flight(data, return_date):
    ...
    # handle empty data if no flight is returned
    if data is None or (not data.get("best_flight") and not data.get("other_flights")):
        print("No Flight Data")
        return FlightData("N/A","N/A","N/A","N/A","N/A",)


    # Combine best_flights and other_flights into one list
    all_flights = data.get("best_flight", []) + data.get("other_flights", [])
    ...
    for flight in all_flights:
        try:
            ...
            print("-- No price available for flights. --")
            continue
